...
class TCN(nn.Module):
    def __init__(self, embed_dim, num_classes, num_channels=[100, 100, 100], kernel_size=3):
        ...
        for i in range(num_levels):
            ...
            layers.append(TCNBlock(in_channels, out_channels, kernel_size, dilation=2**i))
    ...
    def forward(self, x):
        ...
        x = self.tcn(x)
        x = F.adaptive_max_pool1d(x, 1).squeeze(2)  # 全局最大池化
        return x
