...
class TextCNN(nn.Module):
    def __init__(self, embed_dim, num_classes, kernel_sizes=(3, 4, 5), num_filters=100):
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1)  # (batchSize, dim, length)，适应Conv1d格式
        x = [F.relu(conv(x)) for conv in self.convs]  # 多个卷积核提取特征
        # x = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in x]  # Global Max Pooling
        x = [F.max_pool1d(conv, kernel_size=int(conv.shape[2])).squeeze(2) for conv in x]  # Global Max Pooling
        x = torch.cat(x, dim=1)  # 拼接不同窗口大小的特征
        x = self.dropout(x)
        return x # (batchSize, num_classes)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.text_cnn = TextCNN(inchannel, 2)
