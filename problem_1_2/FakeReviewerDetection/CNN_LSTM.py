...
class CNN_LSTM(nn.Module):
    def __init__(self, embed_dim, num_classes, num_filters=100, kernel_sizes=(3, 4, 5), lstm_hidden=128, lstm_layers=1):
        super(CNN_LSTM, self).__init__()
        self.convs = nn.ModuleList([
            nn.Conv1d(in_channels=embed_dim, out_channels=num_filters, kernel_size=k, padding=k//2)
            for k in kernel_sizes
        ])
        self.lstm = nn.LSTM(input_size=num_filters * len(kernel_sizes), hidden_size=lstm_hidden, num_layers=lstm_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):
        ...
        x = torch.cat(x, dim=1)
        x = x.unsqueeze(1)  # (batch_size, 1, num_filters * len(kernel_sizes))
        x, _ = self.lstm(x)
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.cnn_lstm = CNN_LSTM(inchannel, 2)
