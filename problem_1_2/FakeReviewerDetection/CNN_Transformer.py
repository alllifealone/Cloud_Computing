...
class CNN_Transformer(nn.Module):
    def __init__(self, embed_dim,  num_filters=100, kernel_sizes=(3, 4, 5), num_heads=4, trans_layers=2):
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=num_filters * len(kernel_sizes), nhead=num_heads, dim_feedforward=512),
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1)  # 调整维度 (batch_size, embed_dim, seq_len)
        x = [F.relu(conv(x)) for conv in self.convs]
        # x = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in x]
        x = [F.max_pool1d(conv, kernel_size=int(conv.shape[2])).squeeze(2) for conv in x]  # Global Max Pooling
        x = torch.cat(x, dim=1).unsqueeze(1)  # (batch_size, 1, num_filters * len(kernel_sizes))
        x = self.transformer_encoder(x)
        x = self.dropout(x[:, 0, :])  # 取 Transformer 的输出
        return x
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.cnn_transformer = CNN_Transformer(inchannel)
